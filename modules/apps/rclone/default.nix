...
let
  rcloneMountArgs = ''
    ...
    --buffer-size 512M
  '';
  mountOneDrive = "/home/${username}/onedrive/Brain";
in
{
  #----- Home Manager Config -----
  home-manager.users.${username} =
    { pkgs, ... }:
    {
      ...
      systemd.user.services.rclone-onedrive = {
        Unit = {
          Description = "mount onedrive brain dir";
          After = [ "network-online.target" ];
        };
        Install.WantedBy = [ "default.target" ];
        Service = {
          ExecStartPre = "/run/current-system/sw/bin/mkdir -p ${mountOneDrive}";
          ExecStart = "${pkgs.rclone}/bin/rclone mount onedrive:Brain/ ${mountOneDrive}  ${rcloneMountArgs}";
